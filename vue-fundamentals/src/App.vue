<template>
  ...
  <h2 :style="headerStyleObject">Object Styles</h2>
  ...
  <h2 v-for="(name, index) in fullNames" :key="name.first">
  ...
  <!-- v-on: can ve replaced by @ -->
  <button @click="increment(5)">Increment 5</button>
  <button @click="count > 0 && decrement(1)">Decrement 1</button>
  <button @click="count > 4 && decrement(5)">Decrement 5</button>
  <!-- Event Handling -->
  <hr />
  <hr />
  <hr />
  ...
  <!-- Bonus Directives -->
  <!-- used to render only once -->
  <h2 v-once>{{ name }}</h2>
  <button @click="name = 'Batman'">Change name</button>
  <!-- for displaying all the things between opening and closing tags -->
  <h2 v-pre>{{ name }}</h2>
  <!-- Bonus Directives -->
